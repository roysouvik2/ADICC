function f1 = adj_fok_pl(u_1,u_2,kt)
...
% Evaluating the trajectories at the final time.
tr1 = traj1(t(N));
tr2 = traj2(t(N));
...
% Evaluating the trajectories at the current time.
tr1 = traj1(t1);
tr2 = traj2(t1);
...
for i = 2:m
    for j = 2:m
        ...
        b1(l) = fm1(l) - dt*V1(x(i),y(j),tr1,tr2); 
    end
...
for k = N-2:-1:1
    ...
    for i = 2:m
        for j = 2:m
            ...
            % Building the right hand side
            b1(l) = 4*fm1(l) - fm2(l) - 2*dt*V1(x(i),y(j),tr1,tr2); 
        end
